...
5. 启动本地 Canvas Agent，必须带上第 3/4 步得到的真实画布地址：`CANVAS_URL=<真实画布地址> npx -y @yinchenhuang/canvas-agent`。如果 Agent 已经在运行，则读取 `~/.infinite-canvas/canvas-agent.json` 或 `/config` 获取 `Local URL` 和 token。
...
8. 画布网页会自动新建具体画布并连接本地 Agent；不要用浏览器点击新建画布，也不需要手动点击连接。
...
- 不要假设用户已经安装本仓库依赖；插件的 MCP 会通过 `npx -y @yinchenhuang/canvas-agent mcp` 使用已发布的 Canvas Agent。
