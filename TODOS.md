...
- [ ] **控制台 "empty string src" 警告清理(Low,存量)** — 画布/生图页加载时约 14 条 React `<img src="">` 警告。2026-07-24 /qa 核实与 feat/upstream-sync-batch1 无关(分支 diff 零 src 改动)。排查空缩略图/封面渲染,改条件渲染或传 null。证据:.gstack/qa-reports/qa-report-localhost-3000-2026-07-24.md ISSUE-001。
...
- [x] **第二块 · 图像层** — 已完成(2026-07-22,含一次方向返工)。初版按设计实施 Module3 照相级故事板+ST 色板锚定+首帧退役(16d88ab);随后被 skill 更新推翻,返工为 **blockout 粗模**路线(915e608):故事板改为不传资产卡/色板参考的未贴图中性灰模,只锁机位/景别/站位/体块/纵深,**构图与外观在视频层分家**——视频生成由第 1 张故事板锁构图、角色/场景/道具/色板卡锁外观;另补质感样板卡型+布光基准进提示词(ba4932f)、人物资产卡设定页版式(fe2a1f4→fe2a1b4)。**遗留观察项在 pending-test.mdx**:渲染粒度偏精细(接近精细白模,装备数量错时先怀疑此处)、样板/资产卡/灰模稳定性均待更多实测。外部方法参考:docs/reports/depth-map-storyboard-method-20260721.md(深度图分镜,控制信号分工思路,未采用但留档)。
- [x] **第三块 · 视频与交付层** — 已完成(2026-07-24,批A d12bba6 + 批B fb64a65,设计文档 `2026-07-24-toonflow-block3-video-delivery-design.md`)。批A:视频工作台换血 Module4 六段式两步生成(LLM 合成可审文本→确定性校验器→人审→cano),缝合同五行首次被视频层消费,台词类型分流控口型,人声抑制走 prompt 硬编码;禁词按方法论意图收窄(skill 内部冲突裁决:词库"固定位微动"/布光"肖像"与 §2.5/2.6 字面禁词矛盾,词组级+比喻句式拦截,固定段不受禁词审查)。批B:P7 七项质检单(三层门,未清 P0 不得通过)、seam-check 边界卡(五行合同+首尾帧抽帧对照+十条剪辑手法建议)、audio-mix 激活(逐句 TTS 配音轨+计划偏移)、export 可选混音/响度归一(-16 LUFS,无损路径不变)。**实测项全部登记 pending-test.mdx:人声抑制有效性(风险表首项)、口型、缝质量、Module4 合成质量、配音对齐偏差。**
