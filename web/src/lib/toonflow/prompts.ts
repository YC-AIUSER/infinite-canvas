...
P7 导演技法映射：从下方【P7 导演技法映射（九类叙事目的）】逐字选取。先逐字抄一个主目的，最多再抄一个辅目的，并附引用剧本的一句话依据；再从主目的对应行逐字抄取 ≥2 个技法名（辅目的非空时，从主辅两行合计逐字抄取 ≥3 个），禁止改名、简写、拼接或自创。选取后必须逐条转写为可执行指令——运镜写“起点 → 方向 → 速度 → 停点”，构图写主体在哪区、前景遮挡、留白位置、线条如何压迫，灯光写主光方向、逆光/顶光/侧光、阴影落点、色温关系，剪辑写为什么切、切到什么、观众多知道什么，声音写从哪来、提前还是延后、与动作同点还是反差。禁止“配合某技法”“强化某效果”“综合考虑”“灵活运用”这类无参数架空指令。
...
1. mood 字段以“高潮/爆点镜”开头，供下游机械识别；action 字段先写自然中文主动作句，再逐项追加“L5 五部位：头部=...；肩背=...；手部=...；身形=...；身段=...”。五部位每项都必须带角度、幅度、距离、速度或力度参数，禁用“微微/略微/轻微”等弱词。
...
1. “L5 五部位：头部=...；肩背=...；手部=...；身形=...；身段=...”。五部位各带角度、幅度、距离、速度或力度参数，接近生理最大并写出全身/环境反馈；禁用“微微/略微/轻微”等弱词。
