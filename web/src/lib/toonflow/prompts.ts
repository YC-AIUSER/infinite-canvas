...
【参考图分工（必须按文字显式执行）】
参考图列表中第 1 张是该段 Module3 blockout 故事板页，只作为构图基准，只定义镜序、机位、景别、人物左右站位与体块关系、姿态动作、前中后景纵深遮挡，永不覆盖外观。第 2 张起的角色、服装、场景、道具与色板资产卡是外观、装备、场景形态和颜色基准；人物长相、发型、服装和装备一律以角色参考图为准。本渠道会平均分配全部参考图权重，因此必须严格按这里点名的分工执行，不能依赖参考图顺序自行猜测用途。
...
const STAGE_REDLINE_OVERRIDES: Partial<Record<ToonflowNodeKind, string>> = {
    "video-workbench": "视频工作台：参考图列表第1张blockout故事板页只定镜序、机位、景别、左右站位、姿态与纵深遮挡，永不覆盖外观；其余资产卡定人物长相、服装、装备与色彩，逐镜写死谁在画面左、谁在画面右，追加逐字英文外观绑定句与ST色板锚定句；每镜与格子1:1，原生多镜头直出，禁止首尾帧续接或硬拼，不合并镜头、不新增机位。",
    "storyboard-page":
