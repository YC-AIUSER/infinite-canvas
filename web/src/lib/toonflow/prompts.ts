 ...
 * 色板只锁色调，构图/人物外观/装备仍归 Module3 故事板首帧，二者串联不冲突。
 */
...
// 色板锚定的适用面按 03-assets.md §6.3 逐字取：角色板、SCN 场景板、Module3 故事板。
...
/**
 * Module3 照相级故事板（06-video-prompt.md §一）。
 * plus 线弃用「先出黑白粗线稿、再由首帧节点上色」——故事板本身就是该段视频的首帧主参考，
 * 直接按成片画面质量出图，中间不再有线稿环节。
 */
export function buildStoryboardPagePrompt(input: {
    ...
    const panels = rows.map((row, index) => {
        ...
        // 背景只能交由生图侧按空间合同补足，属已知缺口（上游补字段前不自己编内容）。
        const details = [
            `第${index + 1}格（shotNo ${row.shotNo}）`,
            `景别：${row.scale}（本格只有这一个景别）`,
            `机位角度：${row.angle}`,
            `前景（浅灰亮面）：${row.action}${actionContract ? `；停在这一瞬间：${actionContract.process}` : ""}`,
            `中景（中灰暗面）：${shotContract?.subjectRelation?.trim() || "按空间合同补足本镜的中景主体关系，不新增等权重主体"}`,
            "背景（深灰阴影）：按空间合同补足该场景的背景暗示，至多 1 处",
        ];
        if (shotContract) {
    ...
    return `生成该段 Module3 故事板：这张图就是该段视频的首帧主参考，按成片画面质量直接出图，不是草图、不是预演。
...
【人物标签必须携带装备特征】
每个角色都要连同角色卡上的关键装备一起画全——头盔、耳麦、面罩、护具、随身道具，一件都不能省；画面里只凭装备特征就该认出是谁（“头戴四镜筒头盔的瘦高男”，不是一个光着头的“瘦高男”）。这张故事板是首帧主参考，在这里丢掉的装备，后面补不回来。
...
写实漫画渲染（高光点、空气透视、环境光反射、雨水反光细节）、本镜指定室内时出现外景建筑全景、雷电成为画面主体、同一格里合并起手与收手两个动作阶段。
本段第 1 格禁止画成外景大远景建立镜头，也禁止场景空镜与角色静态入画。
写实摄影风格，禁止“漫画”“卡通”“动画”“插画风”“手绘风”“水墨”“Q版”——出现即判风格漂移。

调度信息只是内部构思，不许出现在画面上：因果锚点、主视觉任务标签、空间锚点编号、道具状态链一律不画、不写。画面禁止任何文字、台词、字幕、格号、标签、水印和 logo。同一角色跨格保持外观、装备与屏幕侧一致。
${PALETTE_ANCHOR_SENTENCE}`;
}

/**
 * 首帧（plus 线已退役）。plus 体系里没有「线稿上色」这一步——Module3 故事板本身就是首帧。
 * 保留本函数与 keyframes 这个 kind 纯为兼容旧画布：旧项目（如跑刀）打开与回看历史版本时仍会读到它，删了就打不开。
...
// video-workbench 属第三块(视频层),本轮不动,仍是旧线九宫格表述。
const STAGE_REDLINE_OVERRIDES: Partial<Record<ToonflowNodeKind, string>> = {
    ...
    "storyboard-page":
        "故事板页：出Module3照相级故事板，这张图就是该段首帧主参考，不是线稿预演；16:9横版，画格排列3-5镜3+2、6镜3+3、7-8镜4+4，一格=一镜=一时点=一景别；每格按前景/中景/背景三层空间语法锚定（浅灰亮面/中灰暗面/深灰阴影），禁位置百分比语言与成品渲染词汇；每格只有1个前景主体+至多1个背景暗示；角色必须连关键装备一起画全，禁裸称人物；缝合同画进首末格——末格动作停在中间态、首格接同一动作后半且禁止重新建立空间；禁速度线/残影/同角色同格两次/漫画分镜布局/跨格共享元素/拟声词，禁“漫画/卡通/动画/插画风/手绘风/水墨/Q版”；所有图像生成追加ST色板锚定句（含防泄漏句，不可省）。",
    keyframes: "首帧：plus线已退役，不再有“线稿上色”这一步——Module3故事板本身就是首帧，本环节默认跳过，仅为兼容旧画布保留。",
