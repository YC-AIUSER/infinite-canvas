...
P7 导演技法映射：从下方【P7 导演技法映射（九类叙事目的）】逐字选取。先逐字抄一个主目的，最多再抄一个辅目的，并附引用剧本的一句话依据；再从主目的对应行逐字抄取 ≥2 个技法名（辅目的非空时，从主辅两行合计逐字抄取 ≥3 个），禁止改名、简写、拼接或自创。选取后必须逐条转写为可执行指令——运镜写“起点 → 方向 → 速度 → 停点”，构图写主体在哪区、前景遮挡、留白位置、线条/尺度如何压迫，灯光写主光方向、逆光/顶光/侧光、阴影落点、色温关系（不写“高级光影”），剪辑写为什么切、切到什么、观众多知道/感觉到什么，声音写从哪来、提前还是延后、与动作同点还是反差。禁止“配合某技法”“强化某效果”“综合考虑”“灵活运用”这类无参数架空指令。
...
【高潮/爆点镜三件套（阻断级）】
凡冲突爆点、变身、冲锋、反杀、生死时刻、情绪爆发等高潮/爆点镜，三件套缺一不许输出。字段落点必须固定：
1. mood 字段以“高潮/爆点镜”开头，供下游机械识别；action 字段先写自然中文主动作句，再逐项追加“L5 五部位：头部=...；肩背=...；手部=...；身形=...；身段=...”。五部位各写各自的编码维度（05 §5.2 逐字）：头部=角度+幅度+速度；肩背=紧张度+位移；手部=对象+力度+频率；身形=方向+幅度+距离；身段=整体气质。前四项必须带可观测参数，身段写气质描述不写数值；禁用“微微/略微/轻微”等弱词。
2. action 字段继续追加“Speed Ramp：加速触发=...；顶点=...；慢放区间=...；收束停点=...”，四要素逐项写成该镜可见动作与时间落点，不得只写“快慢变化”。
3. scale 字段只能取 L4 或 L5；不得用 L0-L3 代替关键特写。
...
除每段第一行额外携带上述 p7 外，数组每行必须且只能包含以下基础字段（键名必须逐字使用英文，含义：scale=景别、angle=机位角度、action=动作、line=台词、sfx=音效、mood=情绪、durationSec=时长秒、assetSlots=素材槽位）：
{
    ...
    "assetSlots": []
            ...
            ["hook", "shotScale", "directorTechnique"],
        ),
        context,
    );
}
...
【高潮/爆点镜三件套（阻断级）】
分镜表 mood 标为“高潮/爆点镜”，或动作属于冲突爆点、变身、冲锋、反杀、生死时刻、情绪爆发时，合同必须同时落实：
1. scale 字段只能取 L4 或 L5。
2. movement 字段仍从运镜封闭词库逐字选取，speed 字段仍只写缓/常/急；Speed Ramp 四要素不得伪装成运镜名或速度词，统一写进 endpoint 字段，固定格式为“Speed Ramp：加速触发=...；顶点=...；慢放区间=...；收束停点=...”，最后的收束停点必须是可观察状态。
3. endpoint 里的四阶段必须与动作合同 process 字段的 L5 五部位动作同步；任一缺失都不许输出。
...
【高潮/爆点镜三件套（阻断级）】
分镜表 mood 标为“高潮/爆点镜”，或动作属于冲突爆点、变身、冲锋、反杀、生死时刻、情绪爆发时，process 字段必须一次写满：
1. “L5 五部位：头部=...；肩背=...；手部=...；身形=...；身段=...”。五部位各写各自的编码维度（05 §5.2 逐字）：头部=角度+幅度+速度；肩背=紧张度+位移；手部=对象+力度+频率；身形=方向+幅度+距离；身段=整体气质。前四项必须带可观测参数，身段写气质描述不写数值；整体接近生理最大并写出全身/环境反馈；禁用“微微/略微/轻微”等弱词。
2. “Speed Ramp：加速触发=...；顶点=...；慢放区间=...；收束停点=...”。四要素缺一不可，并与 cause、consequence、endState 的因果链对齐。
3. 同 shotId 的镜头合同 scale 必须为 L4 或 L5；发现 L0-L3 就不许输出该动作合同。
...
5. 严禁复写镜头数量、镜头顺序、构图关系、摄影机位置、切换指令、景别标签；严禁[N-N秒]、Shot/Tn编号、固定机位/锁机/静态机位/定格/一动不动、比喻句式（仿佛/犹如/好似/宛如/如同/就像/好像/像…一样）、逐镜列表。封闭词库词条（如“固定位微动”）按库内原文引用不算违规；逐字抄写下方顿帧两式的完整英文模板不会触发“定格”禁则，但不得改写成中文“定格”。
6. Tone来自本段mood与锁定表；BGM衔接来自剧本F4 BGM行与段情绪，写自然语言且不超过200字。
7. 有入缝时首句接上段同一动作后半且不重新建立空间；有出缝时末句收在动作中间态。

【顿帧两式封闭库（仅 Module4）】
${renderLibraries(["speedFreeze"])}
只在选用“Speed Ramp（加速触发→顶点→慢放区间→收束停点四要素）”时使用，并且只嵌入“顶点”位；按动作类型二选一，完整逐字抄取英文模板，只替换方括号占位内容，禁止自创第三种顿帧。Module3 故事板仍禁止速度线、残影与运动模糊，不得把这两式转写回故事板。
...
const STAGE_REDLINE_OVERRIDES: Partial<Record<ToonflowNodeKind, string>> = {
    "video-workbench": "视频工作台：先用文本模型合成可审的Module4六段文本（参考图索引/故事线/Tone/BGM衔接/风格/画面要求），确定性校验器只审故事线/Tone/BGM三段（模板固定段不受禁词审查），拦时间码、Shot/Tn编号、固定机位/锁机/静态机位/定格、比喻句式、逐镜列表、缺段与错误开头，违规带反馈自动重试1次；通过后用户确认才用该文本调用cano。故事线只写动作/运镜方向/布光方向/音效/台词/表演/衔接词，以“按故事板镜头顺序自然推进”开头，首句覆盖钩子，逐句写死左右站位，严禁复写镜头数量、构图、景别；dialogue只控口型，OS不进故事线且全员闭口。参考图按角色→物品→场景→色板→故事板构图编号，保留外观绑定、参考图分工、布光基准与ST色板锚定句；有入缝首句接同一动作后半且不重新建立空间，有出缝末句收在中间态并限制段尾0.5s持续音；视频只生成环境音效，禁人声/对白/旁白与字幕水印；cano无首帧机制，禁止首尾帧续接或硬拼。视频生成后必须完成P7七项人工质检，七项全勾且无未清P0才可通过。",
    "seam-check": "接缝检查：逐边界按fromSegment/toSegment段序核对directing-lock五行缝合同，并对照上段尾帧与本段首帧；未签合同必须提示回directing-lock补签；按合同景别/动机行高亮剪辑手法建议。Agent或ffmpeg抽帧失败时降级为只核对合同文本，人工勾选流程不得阻塞。",
    "audio-mix": "音频混音：按本段分镜表line提取出口对白与OS，角色音色来自节点voiceMap，OS无角色音色时用旁白音色兜底；按段内前序镜头durationSec累计偏移，逐句TTS并存配音轨，全部成功进入review，人工试听后才可通过；分镜表新版本必须使对应段配音失效。",
    export: "成片导出：默认保持现有无损ffmpeg拼接；仅在用户勾选混入配音轨或响度归一-16 LUFS时转码。混入配音轨须全段audio-mix已人工试听并通过，配音用adelay按偏移对齐后amix；响度归一使用loudnorm I=-16。",
    "storyboard-page":
    ...
    "storyboard-table": "分镜表：只读锁定表、只引用不复判；Layer1 决策前件（剧本原文锚定、上下文感知、黄金三秒钩子六字段、导演技法映射、母题出场、相邻段接缝）缺一不生成；P7 技法映射的结果写进每段 shotNo 最小那行的 p7 字段（primaryPurpose/secondaryPurpose/basis/techniques/transcription），技法名从九类封闭库对应行逐字抄、≥2 个（有辅目的≥3），同段其余行不带 p7；高潮/爆点镜三件套落字段——mood 以「高潮/爆点镜」开头、action 追加 L5 五部位（头部=角度+幅度+速度／肩背=紧张度+位移／手部=对象+力度+频率／身形=方向+幅度+距离／身段=整体气质）与 Speed Ramp 四要素、scale 只取 L4/L5；分段以贴满15s打包连续因果画格为目标，真缝只落在场景/时间转换处；台词按出口对白/OS标注类型，OS段全员闭口。",
