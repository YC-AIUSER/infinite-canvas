...
硬性规则：
1. 换场景或换时间就新建 segment；同一 segment 必须是一个连续场景，段间只在有动机的剧情断点切换。
2. 每镜只有一个明确、可见、可完成的主体动作，不堆叠多个动作或多个运镜；静态人物也必须有可见微动作。
3. 分镜格子与后续视频 prompt 必须 1:1：不合并格子，不编造格子里没有的机位，不用文字强行改变镜头数量。
4. 如输入上下文包含已有 segmentId/shotId 清单，必须逐项原样回传，不得改写、补零、重排或重新编号；新增 segmentId、shotId 一律填写空字符串。
5. 时长按戏剧节拍和动作完成所需分配，避免机械等长；每镜通常承载约 4—6 秒的完整动作落点。
6. 禁止 UI、字幕卡、标题卡和画面内解释文字。
