...
const AGENT_CONNECT_STEPS = [
    ...
    { title: "方式二：直接运行 Agent", text: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", command: "npx -y @yinchenhuang/canvas-agent" },
];
...
export function CanvasLocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    // 撤销按钮只依赖 canUndo 布尔值：canvasContext 对象拖拽时每帧更换，但布尔不变就不触发重渲染
    const canUndo = useAgentStore((state) => Boolean(state.canvasContext?.canUndo));
    ...
    useEffect(() => {
        ...
        source.addEventListener("hello", (event) => {
            ...
            setAgentState({ connected: true, activity: busy ? "Codex 正在运行" : "已连接", waiting: busy, sending: false, connectError: "", messages: useAgentStore.getState().messages.filter((item) => !isConnectionErrorMessage(item)) });
    ...
    // 自愈:结束信号(codex_state busy=false / agent_done)走 SSE 丢失时(如后台标签页节流),前端会
    // 永久卡在 waiting=true——既转圈又发不出新消息。静默超过阈值后探后端 /health,仅当后端确认无
    // 活跃 turn(activeTurns===0)才把界面清回就绪;后端仍在跑(长 turn)则继续等,绝不误伤。判据见 turn-watchdog.ts。
    useEffect(() => {
        if (!connected) return;
    ...
    const runToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        ...
        try {
            ...
            addMessage({
                ...
                text: payload.name === "canvas_apply_ops" ? summarizeCanvasAgentOps(input.ops || []) || "画布操作" : payload.name === "site_navigate" ? `已跳转到 ${input.path || "/"}` : payload.name === "export_stitch" ? stitchToolText(result) : "已完成",
        ...
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                ...
                <div className="rounded-lg border px-3 py-2.5 text-xs leading-5" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                    ...
                    <div className="mt-1">只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 `npx -y @yinchenhuang/canvas-agent` 启动本地 Agent 不会安装 MCP。</div>
