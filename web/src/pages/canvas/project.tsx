...
function ConnectionCreateMenu({
    ...
    // 与 NodeCreateMenu 同款同步锁：菜单卸载前已聚焦按钮可被回车重复触发，
    // 一次菜单实例只放行一次创建（菜单条件渲染，关闭即卸载，重开自然复位）
    const creatingRef = useRef(false);
    const handleCreate = (type: Parameters<typeof onCreate>[0]) => {
    ...
    return (
        ...
        >
            ...
            <div className="grid gap-1">
                <ConnectionCreateOption theme={theme} icon={<List className="size-5" />} title="文本生成" description="脚本、广告词、品牌文案" onClick={() => handleCreate(CanvasNodeType.Text)} />
                <ConnectionCreateOption theme={theme} icon={<ImageIcon className="size-5" />} title="图片生成" onClick={() => handleCreate(CanvasNodeType.Image)} />
                <ConnectionCreateOption theme={theme} icon={<Video className="size-5" />} title="视频生成" onClick={() => handleCreate(CanvasNodeType.Video)} />
                <ConnectionCreateOption theme={theme} icon={<Music2 className="size-5" />} title="音频参考" onClick={() => handleCreate(CanvasNodeType.Audio)} />
                <ConnectionCreateOption theme={theme} icon={<Settings2 className="size-5" />} title="配置节点" description="模型、尺寸、数量和输入顺序" onClick={() => handleCreate(CanvasNodeType.Config)} />
            </div>
        </div>
    );
}
...
function NodeCreateMenu({ position, onCreate, onClose }: { position: Position; onCreate: (type: CanvasNodeType) => void; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            <div className="grid gap-1">
                <ConnectionCreateOption theme={theme} icon={<List className="size-5" />} title="文本" onClick={() => handleCreate(CanvasNodeType.Text)} />
                <ConnectionCreateOption theme={theme} icon={<ImageIcon className="size-5" />} title="图片" onClick={() => handleCreate(CanvasNodeType.Image)} />
                <ConnectionCreateOption theme={theme} icon={<Video className="size-5" />} title="视频" onClick={() => handleCreate(CanvasNodeType.Video)} />
                <ConnectionCreateOption theme={theme} icon={<Music2 className="size-5" />} title="音频" onClick={() => handleCreate(CanvasNodeType.Audio)} />
                <ConnectionCreateOption theme={theme} icon={<Settings2 className="size-5" />} title="生成配置" onClick={() => handleCreate(CanvasNodeType.Config)} />
                <ConnectionCreateOption theme={theme} icon={<Group className="size-5" />} title="组" onClick={() => handleCreate(CanvasNodeType.Group)} />
            </div>
        </div>
    );
}
...
function InfiniteCanvasPage() {
    ...
    const isProjectActive = useCallback((capturedProjectId: string) => capturedProjectId === projectIdRef.current && loadedProjectIdRef.current === capturedProjectId, []);
    const guardProject = useCallback(
        (capturedProjectId: string) => {
            if (isProjectActive(capturedProjectId)) return true;
            message.warning("画布已切换，本次异步结果已丢弃");
            return false;
        },
        [isProjectActive, message],
    );
    ...
    const cancelStreamNodeUpdate = useCallback((nodeId: string) => {
        const timer = streamUpdateTimersRef.current.get(nodeId);
        if (timer) clearTimeout(timer);
        streamUpdateTimersRef.current.delete(nodeId);
        pendingStreamTextRef.current.delete(nodeId);
    }, []);

    const writeStreamNodeContent = useCallback((nodeId: string, content: string) => {
        setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, type: CanvasNodeType.Text, metadata: { ...node.metadata, content, status: NODE_STATUS_LOADING } } : node)));
    }, []);

    const scheduleStreamNodeUpdate = useCallback(
        (nodeId: string, content: string) => {
            pendingStreamTextRef.current.set(nodeId, content);
            if (streamUpdateTimersRef.current.has(nodeId)) return;

            streamUpdateTimersRef.current.set(
                nodeId,
                setTimeout(() => {
                    streamUpdateTimersRef.current.delete(nodeId);
                    const pending = pendingStreamTextRef.current.get(nodeId);
                    pendingStreamTextRef.current.delete(nodeId);
                    if (pending !== undefined) writeStreamNodeContent(nodeId, pending);
    ...
    const flushStreamNodeUpdate = useCallback(
        (nodeId: string, content: string) => {
            const timer = streamUpdateTimersRef.current.get(nodeId);
            if (timer) clearTimeout(timer);
            streamUpdateTimersRef.current.delete(nodeId);
            pendingStreamTextRef.current.delete(nodeId);
            writeStreamNodeContent(nodeId, content);
        },
        [writeStreamNodeContent],
    );
    ...
    useEffect(() => {
        ...
        streamUpdateTimersRef.current.forEach((timer) => clearTimeout(timer));
        streamUpdateTimersRef.current.clear();
        pendingStreamTextRef.current.clear();
        toonflowStreamTimersRef.current.forEach((timer) => clearTimeout(timer));
        toonflowStreamTimersRef.current.clear();
    ...
    const markUserViewportInteraction = useCallback(() => {
        lastManualViewportAtRef.current = Date.now();
        cancelCameraAnimation();
    }, [cancelCameraAnimation]);

    const focusNodes = useCallback(
        (targetNodes: CanvasNodeData[]) => {
            if (Date.now() - lastManualViewportAtRef.current < VIEWPORT_CAMERA_COOLDOWN) return;
            const bounds = computeNodesBounds(targetNodes);
            if (!bounds) return;
            const rect = containerRef.current?.getBoundingClientRect();
            const target = computeFocusViewport(bounds, { width: rect?.width || size.width, height: rect?.height || size.height }, viewportRef.current);
            if (!target) return;
            ...
            const animate = (now: number) => {
                const progress = Math.min((now - startedAt) / VIEWPORT_CAMERA_DURATION, 1);
                const next = interpolateViewport(from, target, easeInOutCubic(progress));
        ...
        [cancelCameraAnimation, size.height, size.width],
    );
    ...
    const applyAgentOps = useCallback(
        (ops?: CanvasAgentOp[]) => {
            ...
            focusNodes(next.nodes.filter((node) => !beforeNodeIds.has(node.id)));
        ...
        [currentProject?.title, focusNodes, projectId],
    );
    ...
    useEffect(() => {
        ...
        const configMetadata: CanvasNodeMetadata =
            skill.flow.mode === "text" ? { model: effectiveConfig.model } : { model: effectiveConfig.imageModel || effectiveConfig.model, size: effectiveConfig.size, count: getGenerationCount(effectiveConfig.canvasImageCount || effectiveConfig.count) };
    ...
    const resetViewport = useCallback(() => {
        ...
        setViewport(next);
        setContextMenu(null);
    }, [markUserViewportInteraction, size.height, size.width]);
    ...
    const createImageFileNode = useCallback(async (file: File, position: Position, activate = true, capturedProjectId = projectIdRef.current) => {
        const image = await uploadImage(file);
        if (!guardProject(capturedProjectId)) return;
    ...
    const createVideoFileNode = useCallback(async (file: File, position: Position, activate = true, capturedProjectId = projectIdRef.current) => {
    ...
    const createAudioFileNode = useCallback(async (file: File, position: Position, activate = true, capturedProjectId = projectIdRef.current) => {
    ...
    const pasteSystemClipboard = useCallback(async () => {
        ...
        if (imageItem) {
            ...
            void createImageFileNode(file, getCanvasCenter(), true, capturedProjectId);
    ...
    }, [createImageFileNode, createTextNodeFromClipboard, getCanvasCenter, guardProject, message]);
            ...
            if (target?.nodeId) {
                ...
                void (async () => {
                    for (const [index, item] of files.entries()) {
                        ...
                        try {
                            await (kind === "audio" ? createAudioFileNode(item, position, activate, capturedProjectId) : kind === "video" ? createVideoFileNode(item, position, activate, capturedProjectId) : createImageFileNode(item, position, activate, capturedProjectId));
                        } catch {
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                if (mode === "image") {
                    ...
                    focusNodes([rootNode, ...childNodes]);
                    ...
                    await Promise.all(
                        targetIds.map(async (targetId) => {
                            try {
                                ...
                                const uploaded = await uploadImage(image.dataUrl);
                                if (!isProjectActive(capturedProjectId)) return false;
                ...
                const answers = await Promise.all(
                    textTargetIds.map((targetNodeId) => {
                        ...
                        return requestImageQuestion(
                            ...
                            (text) => {
                                if (!isProjectActive(capturedProjectId)) return;
                                localStreamed = text;
                                streamed = text;
                                scheduleStreamNodeUpdate(targetNodeId, text);
                            },
                            { signal: controller.signal },
                        )
                            .then((answer) => {
                                const content = answer || localStreamed;
                                if (isProjectActive(capturedProjectId)) flushStreamNodeUpdate(targetNodeId, content);
                                return { nodeId: targetNodeId, content };
                            })
                            .finally(() => {
                                cancelStreamNodeUpdate(targetNodeId);
        ...
        async (nodeId: string): Promise<{ ok: boolean; error?: string }> => {
            ...
            try {
                ...
                for (let attempt = 0; attempt < 2; attempt += 1) {
                    const rawText = await requestImageQuestion(generationConfig, [{ role: "user", content: finalPrompt }], (text) => {
                        if (isProjectActive(capturedProjectId)) scheduleToonflowStreamUpdate(nodeId, text);
                    }, { signal: controller.signal });
                ...
                if (!guardProject(capturedProjectId)) return { ok: false, error: "画布已切换" };
        ...
        async (nodeId: string, note?: string) => {
            ...
            try {
                ...
                for (let attempt = 0; attempt < 2; attempt += 1) {
                    ...
                    const rawText = await requestImageQuestion(generationConfig, [{ role: "user", content: generation.finalPrompt }], (text) => {
                        if (isProjectActive(capturedProjectId)) scheduleToonflowStreamUpdate(nodeId, text);
                    }, { signal: controller.signal });
        ...
        async (nodeId: string) => {
            ...
            try {
                ...
                for (const item of plan) {
                    ...
                    if (!guardProject(capturedProjectId)) {
                        await deleteStoredMedia([...createdKeys, uploaded.storageKey]);
                        return;
                    }
        ...
        resumableNodes.forEach((resumableNode) => {
            ...
            void (async () => {
                ...
                try {
                    ...
                    if (!guardProject(capturedProjectId)) {
                        if (storageKey) await deleteStoredMedia([storageKey]).catch(() => undefined);
                        return;
                    }
    ...
    }, [finishGenerationRequest, guardProject, message, projectId, projectLoaded, startGenerationRequest]);
    ...
    const handleToonflowAssetCardGenerate = useCallback(
        ...
        [effectiveConfig, guardProject, isAiConfigReady, message, openConfigDialog],
    );
    ...
    const handleAssetCardNodeRegenerate = useCallback(
        async (childNode: CanvasNodeData) => {
            ...
            if (!newKey) return;
            if (!guardProject(capturedProjectId)) {
                await deleteStoredImages([newKey]);
                return;
            }
            const nextCards = cards.map((item) => (item.cardId === ref.cardId ? { ...item, storageKey: newKey } : item));
            await handleToonflowAssetCardsSave(ref.stageNodeId, nextCards);
        },
        [guardProject, handleToonflowAssetCardGenerate, handleToonflowAssetCardsSave],
    );
    ...
    const insertAssistantImage = useCallback(
        ...
        [guardProject, screenToCanvas, size.height, size.width],
    );
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                <InfiniteCanvas
                    ...
                    onUserViewportInteraction={markUserViewportInteraction}
                ...
                {isMiniMapOpen ? <Minimap nodes={nodes} viewport={viewport} viewportSize={size} onViewportChange={handleManualViewportChange} /> : null}
