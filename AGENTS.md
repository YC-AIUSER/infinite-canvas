...
本项目的 `backlog/` 目录和 `backlog` 命令归 Claude 独占。项目 `CLAUDE.md` 里的 Backlog.md 说明是写给 Claude 的，不适用于 Codex。

- 不要建任务、不要改状态、不要勾 AC、不要编辑 `backlog/**` 下任何文件。你是被验收方，自己改状态等于自己判合格。理由同「不碰 git」。
- 简报里若给了任务 ID（形如 `TASK-12`），在最终报告开头原样带回即可，不要读或改那个任务文件。